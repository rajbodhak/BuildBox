    ...
    return (
        <div className="flex flex-col flex-1 px-6 py-10 max-w-lg mx-auto w-full">
            ...
            <div className="flex items-baseline justify-between mb-1">
                <h1 className="text-2xl font-bold text-(--text-h) tracking-tight">✓ Todo App</h1>
                {todos.length > 0 && (
            ...
            <p className="font-mono text-xs text-(--text) mb-8">Built by demo</p>
            ...
            <ul className="flex flex-col gap-2">
                {todos.length === 0 && (
                    <li className="font-mono text-sm text-(--text) text-center py-12 border border-dashed border-(--border) rounded-xl">
                ...
                {todos.map((todo) => (
                    <li
                        key={todo.id}
                        className="flex items-center gap-3 px-4 py-3 rounded-xl border border-(--border) bg-(--code-bg)"
                    >
                        <input
                            type="checkbox"
                            checked={todo.done}
                        ...
                        />
                        <span
                            className={`flex-1 text-sm text-(--text-h) transition-opacity ${todo.done ? 'line-through opacity-40' : ''
                                }`}
                        >
                            {todo.text}
                        </span>
            ...
            {todos.some((t) => t.done) && (
                <button
                    ...
                    className="font-mono text-xs text-(--text) hover:text-(--accent) transition-colors mt-4 self-end"
                >
                    Clear completed
                </button>
            )}
